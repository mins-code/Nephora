...
export default function ResultsPage() {
  ...
  if (!state?.prediction) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-on-background font-sans gap-6">
        <Icon name="error" size="text-5xl" className="text-error" />
  ...
  const { risk_probability, risk_label, risk_color, shap_values, feature_values, creat_slope, n_visits } = prediction
  ...
  return (
    <div className="min-h-screen text-on-background font-sans flex flex-col items-center pt-28 pb-24 px-6">
      <div className="w-full max-w-4xl">

        {/* Step indicator */}
        <div className="flex items-center gap-3 mb-10">
          <Link to="/upload" className="flex items-center gap-2 text-outline hover:text-on-surface transition-colors text-body-md">
            <Icon name="arrow_back" size="text-lg" /> Back to Upload
          </Link>
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full"
            ...
            <span className="text-label-sm text-primary">Step 2 of 2 — CKD Risk Assessment</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 mb-8">
          ...
          <div className="col-span-12 md:col-span-5 rounded-3xl p-8 flex flex-col items-center justify-center text-center"
            ...
            <div className="font-bold leading-none mb-3" style={{ fontSize: '80px', color: riskStyle.color,
              textShadow: `0 0 40px ${riskStyle.color}60` }}>
            ...
            <p className="text-body-md text-outline mt-4">
              ...
              {creat_slope !== 0 && ` · Creatinine slope: ${creat_slope > 0 ? '+' : ''}${creat_slope?.toFixed(3)}`}
            </p>
          </div>
        ...
        <div className="glass-stroke-thin rounded-3xl p-8 mb-8"
          ...
          <p className="text-body-md text-outline mb-6">
            The following biomarkers contributed most to this prediction. Positive values push risk higher; negative values reduce it.
          </p>
          <div className="space-y-3">
            {shapEntries.map(([feature, value]) => {
              const isPositive = value >= 0
              const barWidth = (Math.abs(value) / maxShap) * 100
              const label = feature.replace(/^(mean_val_|max_val_|ever_abnormal_)/, '').replace('creat_slope', 'Creatinine Slope')
              return (
                <div key={feature}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-body-md text-on-surface-variant">{label}</span>
                    <span className="text-label-sm font-semibold" style={{ color: isPositive ? '#ffb4ab' : '#94d3be' }}>
        ...
        {/* ── Visit Timeline ──────────────────────────────────────────────── */}
        {visits?.length > 0 && (
          <div className="glass-stroke-thin rounded-3xl p-8"
            style={{ background: 'rgba(26,32,36,0.6)', backdropFilter: 'blur(20px)' }}>
                  ...
                  <div className="flex-1 pb-4">
                    ...
                    {extractedData?.missing?.length > 0 && (
                      <p className="text-[11px] text-outline mt-2">
                        Not found: {extractedData.missing.join(', ')}
                      </p>
                    )}
                  </div>
                </div>
        ...
        {/* Disclaimer */}
        <div className="mt-8 flex items-start gap-3 px-5 py-4 rounded-2xl"
          ...
          <p className="text-body-md text-on-surface-variant">
            ...
            This result is not a medical diagnosis. Consult a qualified nephrologist for clinical interpretation.
          </p>
        </div>
